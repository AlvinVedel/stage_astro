...
# Définir le modèle sur chaque GPU
"""
with strategy.scope():
    # Création des modèles
    teacher_backbone_strat = Backbone()#create_teacher_backbone_model()
    teacher_head_strat = Head(576, 150)#create_teacher_head_model()
    student_backbone_strat = Backbone()#create_student_backbone_model()
    student_head_strat = Head(576, 150)#create_student_head_model()
"""
with tf.device("/GPU:1"):
    teacher_head = Head(576, 150)#teacher_head_strat
    student_head = Head(576, 150)# student_head_strat

with tf.device("/GPU:2") :
    teacher_backbone = Backbone()#teacher_backbone_strat

with tf.device("/GPU:3") :
    student_backbone = Backbone()#student_backbone_strat


#teacher_backbone(np.random.random((32, 64, 64, 9)))
#teacher_backbone.save_weights("./backbone.weights.h5")
#teacher_backbone.load_weights('./backbone.weights.h5')
...
data_generator = ImageDataGenerator("/home/barrage/HSC_READY_CUBES/XMM_SHALLOW_UNSUP_200K_OBJECTS.npz", 8, (48, 48), (16, 16))

#batch_data = data_generator.__getitem__(0)
#larges = batch_data["large_crop"]
#print(larges.shape)
#larges = batch_data["small_crop"]
#print(larges.shape)
#toto()
...
def compute_koleo_loss(student_outputs) :
    ...
    # Normalisation L2 :
    student_outputs = student_outputs / tf.math.sqrt(tf.reduce_sum((student_outputs)**2, axis=1, keepdims=True)) # batch, 576 / batch, 1 => batch, 576
    ...
    min_pairwise_dist = tf.math.sqrt(tf.reduce_min(dist_mat, axis=1))  # on a un matrice symétrique normalement alors axis pas important, on prend les distances min
...
def update_teacher_model(student_model, teacher_model, momentum):
    ...
    is_nan = False
    for w in new_teacher_weights :
        if tf.reduce_any(tf.math.is_nan(w)):
            is_nan = True
    print("new weights", is_nan)
...
for epoch in range(1) :
    for batch in  data_generator: 
        ...
        with tf.GradientTape() as tape :
                ...
                print("large crops : ",  tf.reduce_any(tf.math.is_nan(large_crops)))
                print(large_crops.shape)
                with tf.device("/GPU:2"):
                    print("........je call le teacher backbone ligne 127")
                    teacher_output_dict = teacher_backbone(large_crops, training=False)  # DINO LOCAL, DINO GLOBAL 
                    #print(teacher_output_dict)
                with tf.device("/GPU:3") :
                    print("........je call le student backbone ligne 131")
                    student_output_dict = student_backbone(large_crops, training=True)  # DINO GLOBAL, KOLEO
                    #print(student_output_dict)

                print("backbone results :", tf.reduce_any(tf.math.is_nan(teacher_output_dict["cls_token"])), tf.reduce_any(tf.math.is_nan(teacher_output_dict["patch_token"])),
                      tf.reduce_any(tf.math.is_nan(student_output_dict["cls_token"])), tf.reduce_any(tf.math.is_nan(student_output_dict["patch_token"])))
                

                # 2  CLS TOKENS PASSAGE DANS HEAD  :   
                teacher_cls_token = teacher_output_dict["cls_token"]  # batch*2, 576
                with tf.device("/GPU:1"):
                    print("........je call le teacher head ligne 142")
                ...
                with tf.device("/GPU:1") :
                    print("........je call le student head ligne 148")
                ...
                print("cls after head :", tf.reduce_any(tf.math.is_nan(teacher_cls_tokens_after_head)), tf.reduce_any(tf.math.is_nan(student_cls_tokens_after_head)))
                ...
                print("DINO LOSS :",dino_global_loss)
                ...
                print("vues réduites :", tf.reduce_any(tf.math.is_nan(small_crops)),  tf.reduce_any(tf.math.is_nan(student_local_output_dict["cls_token"])),  tf.reduce_any(tf.math.is_nan(student_local_cls_tokens_after_head)), tf.reduce_any(tf.math.is_nan(dino_centre)))
                ...
                print("DINO LOCAL LOSS", dino_local_loss)
                ...
                print("LOSS KOLEO", koleo_loss)


                #print("c'est dans le ibot que je bug")

                # 5  RECUPERATION PATCH TOKENS MASQUES TEAHCER
                ...
                with tf.device("/GPU:3") :
                    print("........je call le student backbone ligne 204")
                    masked_student_output_dict = student_backbone(masked_large, training=True)
                ...
                with tf.device("/GPU:2"):
                    print("........je call le teacher head ligne 214")
                    teacher_patch_tokens_after_head = teacher_head(tf.reshape(masked_teacher_patch_tokens, (-1, tf.shape(masked_teacher_patch_tokens)[-1])), training=False)
                with tf.device("/GPU:3"):
                    print("........je call le student head ligne 217")
                    student_patch_tokens_after_head = student_head(tf.reshape(masked_student_patch_token, (-1, tf.shape(masked_student_patch_token)[-1])), training=True)
                # les 2 sont de shape batch * 2 * num_patch, 150
                print("patch masqués :", tf.reduce_any(tf.math.is_nan(masked_large)), tf.reduce_any(tf.math.is_nan(masked_student_patch_token)),
                      tf.reduce_any(tf.math.is_nan(masked_teacher_patch_tokens)), tf.reduce_any(tf.math.is_nan(teacher_patch_tokens_after_head)), tf.reduce_any(tf.math.is_nan(student_patch_tokens_after_head)))
                ...
                print("LOSS IBOT",ibot_loss)
                total_loss = dino_global_loss + dino_local_loss + 0.1*koleo_loss + ibot_loss
                if tf.reduce_any(tf.math.is_nan(total_loss)) :
                    print("NAN dans loss")
                    break
                
        print(total_loss, dino_global_loss, dino_local_loss, koleo_loss, ibot_loss)
        ...
        for g in student_backbone.trainable_variables :
            if tf.reduce_any(tf.math.is_nan(g)) :
                is_nan_bv = True

        for g in student_head.trainable_variables :
            if tf.reduce_any(tf.math.is_nan(g)) :
                is_nan_hv = True
        print("gradients et var", is_nan_g, is_nan_bv, is_nan_hv)
        bweigths_nan = any([tf.reduce_any(tf.math.is_nan(g.weights)) for g in student_backbone.layers])
        print("poids avant gradient :", bweigths_nan)
        print("G = ", gradients)
        bweigths_nan = any([tf.reduce_any(tf.math.is_nan(g.weights)) for g in student_backbone.layers])
        print("poids après gradient :", bweigths_nan)
        ...
        print("centres", tf.reduce_any(tf.math.is_nan(ibot_centre)), tf.reduce_any(tf.math.is_nan(dino_centre)))
