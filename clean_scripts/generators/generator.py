...
### GENERATEURS UTILISES DANS METHODES CONTRASTIVES ET FINETUNING
...
class MultiGen(tf.keras.utils.Sequence):
    def __init__(self, paths, batch_size, do_color=True, do_seg=False, do_mask_band=False, do_adversarial=False, image_size=(64, 64, 6), shuffle=True, extensions=[".npz"], same_samples=True, n_samples=40000):
        self.paths = []
...
### UTILISE SI DONNES DEJA CHARGEES
class DataGenerator(tf.keras.utils.Sequence):
    def __init__(self, image_data, ebv, label, zspec, batch_size=32, augment_prob=0.9, augment_factor=0.1):
    ...
    def __getitem__(self, index):
        # Crée un batch de données
        batch_indices = self.indices[index * self.batch_size:(index + 1) * self.batch_size]
        images = self.image_data[batch_indices]
        ebv = self.ebv[batch_indices]
        label = self.label[batch_indices]
        zspec = self.zspec[batch_indices]
        
        # Application d'une augmentation aléatoire sur les scalaires
        #aug_ebv = self.apply_augmentation(ebv)
        
        return [images, ebv], [label, zspec]
    
    def apply_augmentation(self, scalars):
        # Application de l'augmentation sur les scalaires avec une probabilité
        augmented_scalars = np.copy(scalars)
        mask = np.random.rand(len(scalars)) < self.augment_prob
        noise = np.random.choice([-self.augment_factor, self.augment_factor], size=len(scalars))
        augmented_scalars[mask] *= (1+noise[mask])
        return augmented_scalars
