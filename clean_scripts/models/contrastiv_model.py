...
class simCLR(keras.Model) :
    """
    simCLR avec toutes les options liées au domaine (pas utilisé en pratique) de segmentation, auto encoder, prédiction de couleur, adversarial entre surveys
...
class simCLRcolor1(keras.Model) :
    """
    simCLR avec prédiction auxiliaire de la couleur
    """
...
class simCLRcolor1_adversarial(keras.Model) :
    def __init__(self, backbone, head, color_head, adversarial_net, temp=0.7) :
        """
        simCLR avec loss adversarial entre surveys
        """
        super().__init__()
        self.backbone = backbone
        self.head = head
