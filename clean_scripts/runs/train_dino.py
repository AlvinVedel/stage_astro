...
from models.vit_layers import BackboneAstro, Head
from generators.dino_generator import DinoGenerator, DinoColor
import os


"""

SCRIPT POUR L ENTRAINEMENT DE DINOv2: Learning Robust Visual Features without Supervision
...
batch_size=32
masking_rate = 0.3
data_generator = DinoColor("/lustre/fswork/projects/rech/kof/uve94ap/CUBES_HSC/PHOT/COSMOS", batch_size, (48, 48), (16, 16))
...
def color_predictor() :
      inp = tf.keras.Input((576,))
      l1 = tf.keras.layers.Dense(1024)(inp)
      l1 = tf.keras.layers.PReLU()(l1)
      pred = tf.keras.layers.Dense(8, activation='linear')(l1)
      return tf.keras.Model(inp, pred)
        
color_head = color_predictor()
...
for epoch in range(total_epoch) :
        for batch in  data_generator: 
            ...
            colors = batch["colors"]
            colors_target = tf.concat([colors, colors.copy()], axis=0)  # 32 + 32 = 64
            ...
            with tf.GradientTape() as tape :
                    ...
                    total_loss = dino_global_loss + dino_local_loss + ibot_loss + 0.1*koleo_loss + color_loss
        ...
        if epoch % 1 == 0 :
            print("weights saved")
            teacher_backbone.save_weights("checkpoints_dino_color/teacher_backbone.weights.h5")
            student_backbone.save_weights("checkpoints_dino_color/student_backbone.weights.h5")
            teacher_head.save_weights("checkpoints_dino_color/teacher_head.weights.h5")
            student_head.save_weights("checkpoints_dino_color/student_head.weights.h5")
            teacher_ibot_head.save_weights("checkpoints_dino_color/teacher_ibot_head.weights.h5")
            student_ibot_head.save_weights("checkpoints_dino_color/student_ibot_head.weights.h5")
            color_head.save_weights("checkpoints_dino_color/color_head.weights.h5")
