...
class PatchExtractor(tf.keras.layers.Layer) :
    def __init__(self, patch_size=4, embed_size=512, image_size=64) :
        ...
        self.patch_conv = tf.keras.layers.Conv2D(filters=self.embed_size, kernel_size=(int(self.patch_size), int(self.patch_size)), strides=(int(self.patch_size), int(self.patch_size)), padding='valid', activation='linear')
...
class ViT_backbone(tf.keras.layers.Layer) :
    def __init__(self, embed_dim=256, num_blocks=4, num_heads=8, patch_size=4, gp='average') :
        super().__init__()
        self.embed_dim=embed_dim
        self.patch_master = PatchExtractor(patch_size=patch_size, embed_size=self.embed_dim, image_size=64)
