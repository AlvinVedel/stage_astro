...
def backbone(bn=True, return_all=False) :
    inp = keras.Input((64, 64, 5))
    c1 = layers.Conv2D(96, padding='same', strides=1, kernel_size=3, kernel_regularizer=l2(1e-4))(inp) # 64
    c1 = layers.PReLU()(c1) 
    c2 = layers.Conv2D(96, padding='same', kernel_size=3, strides=1, activation='tanh', kernel_regularizer=l2(1e-4))(c1)  #64
    p1 = layers.AveragePooling2D((2, 2))(c2)  # 32
    c3 = layers.Conv2D(128, padding='same', strides=1, kernel_size=3, kernel_regularizer=l2(1e-4))(p1)
    c3 = layers.PReLU()(c3)
    c4 = layers.Conv2D(128, padding='same', kernel_size=3, strides=1, kernel_regularizer=l2(1e-4))(c3)  #32
    c4 = layers.PReLU(name='c4')(c4) 
    p2 = layers.AveragePooling2D((2, 2))(c4)  # 16
    c5 = layers.Conv2D(256, padding='same', strides=1, kernel_size=3, kernel_regularizer=l2(1e-4))(p2) #16
    c5_bn = layers.PReLU()(c5)
    #c5_bn = layers.BatchNormalization()(c5)
    c6 = layers.Conv2D(256, padding='same', kernel_size=3, strides=1, kernel_regularizer=l2(1e-4))(c5_bn)  #16
    c6 = layers.PReLU()(c6)
    p3 = layers.AveragePooling2D((2, 2))(c6) # 8
    c7 = layers.Conv2D(256, kernel_size=3, strides=1, padding='valid', kernel_regularizer=l2(1e-4))(p3) # 6
    c7 = layers.PReLU()(c7)
    c8 = layers.Conv2D(256, kernel_size=3, strides=1, padding='valid', kernel_regularizer=l2(1e-4))(c7) # 4
    c8 = layers.PReLU()(c8)
    c9 = layers.Conv2D(256, padding='valid', kernel_size=3, strides=1, kernel_regularizer=l2(1e-4))(c8) # 2, 2, 256
    c9 = layers.PReLU()(c9)
...
model = simCLR(backbone(return_all=False), mlp(1024))
#model = simCLR(treyer_backbone(bn=True), mlp(1024))
model.compile(optimizer=keras.optimizers.Adam(1e-3), loss=ContrastivLoss())
model(np.random.random((32, 64, 64, 5)), return_all=False)
for layer in model.backbone.layers :
    print(layer.name)
model.load_weights("../model_save/checkpoints_simCLR_UD_D/simCLR_kernelregu_cosmos_bnTrue_100.weights.h5")
...
"""
back2 = backbone(return_all=True)
back2.set_weights(model.backbone.get_weights())
c1, c2, c3, c4, c5, c6, c7, c8, c9, flat, l1, l1_bn = back2(data_gen.images[:200, :, :, :5])
...
import random
i = random.randint(0, c2.shape[0])
c2_0 = np.reshape(c2, (c2.shape[0], -1))[i]
plt.scatter(np.arange(len(c2_0)), c2_0)
plt.ylim((-1.05, 1.05))
plt.savefig("../plots/distribution_tanh.png")
toto()
"""
iter = 10
while iter <= 1000 :
    ...
    data_gen._load_data()
    if iter % 10 == 0 :
        filename = "../model_save/checkpoints_simCLR_UD_D/simCLR_kernelregu_cosmos_bn"+str(bn)+"_"+str(iter*10)+".weights.h5"
        model.save_weights(filename)  # 6000 minutes   ==> 15 fois 100 épochs

        test_mod = simCLR(backbone(return_all=True),mlp(1024))
        test_mod(np.random.random((32, 64, 64, 5)), return_all=True)
        test_backbone = test_mod.backbone
        test_backbone.set_weights(model.backbone.get_weights())
        c1, c2, c3, c4, c5, c6, c7, c8, c9, flat, l1, l1_bn = test_backbone(data_gen.images[:200, :, :, :5])
        print("1", np.min(c1), np.max(c1), np.var(c1))
        print("2", np.min(c2), np.max(c2), np.var(c2))
        print("3", np.min(c3), np.max(c3), np.var(c3))
        print("4", np.min(c4), np.max(c4), np.var(c4))
