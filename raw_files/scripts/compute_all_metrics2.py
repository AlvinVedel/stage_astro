...
plots_name = "NORM_COLOR_NOBN_HEADPRE"
path_memory = {}
import gc
...
for i, finetune_base in enumerate(["base1", "base2", "base3"]) :   #### POUR CHAQUE CONDITION D ENTRAINEMENT ON VA AVOIR UN SUBPLOT
    ...
    fig1, ax1 = plt.subplots(nrows=len(base_liste), ncols=3, figsize=(50, 25)) # pour 1 treyer + 3 simCLR dans 2 conditions modèles   ==>   heatmap
    fig2, ax2 = plt.subplots(nrows=len(base_liste), ncols=3, figsize=(50, 25)) # ===> BIAS
    fig3, ax3 = plt.subplots(nrows=len(base_liste), ncols=3, figsize=(50, 25)) # ===> SMAD
    fig4, ax4 = plt.subplots(nrows=len(base_liste), ncols=3, figsize=(50, 25)) # ===> OUT2
    ...
    for j, inf_base in enumerate(base_liste):
        ...
        simbases = ["norm300_ColorHead_Regularized"] #    , "UD800_classif","UD_D800_classif"]
        iter=0
        #model_liste = ["simCLR_finetune/simCLR_finetune_"+cond+"_base="+finetune_base+"_model="+sim_base+".weights.h5"]
        prefixe = ["_HeadPre0", "", "_HeadPre10"]
        for k in range((len(simbases))*3) :
            if False :
                ...
                if sim_base == "280_ViTback_ColorHead" :
                    ...
                    model = AstroFinetune(basic_backbone(full_bn=False), astro_head(1024, 400))
                    model(np.random.random((32, 64, 64, 6)))
