...
head = noregu_projection_mlp(2048, True, 128)
backbone(np.random.random((32, 64, 64, 6)))
head(np.random.random((32, 2048)))
...
end_name = "backbone_simMoco_5k_20iter.weights.h5"
end_head_name = "head_simMoco_5k_20iter.weights.h5"
save_w_path = "model_save/simCLR_finetune_comparaison/resnet50_color_t01_moco5k"

#for condition in ["fine", "finecon","coretuning"] :
for condition in ["coretuning"] :
        ...
        if condition == "fine" :
            ...
            for ep in range(n_epochs) :
                ...
                for batch in data_gen :
                    images, labels_dict = batch
                    with tf.GradientTape() as tape :
                        x = backbone(images, training=True)
                        pdf, reg = classifier(x, training=True)
                        ...
                        total_loss = pdf_loss+reg_loss
                    gradients = tape.gradient(total_loss, backbone.trainable_variables+classifier.trainable_variables)
                    optim.apply_gradients(zip(gradients, backbone.trainable_variables+classifier.trainable_variables))
                print("finetune epoch", ep, total_loss, pdf_loss, reg_loss)

            model_to_save = AstroFinetune(backbone, classifier)
            model_to_save.save_weights(base_path+save_w_path+condition+"_"+base+".weights.h5")
        ...
        if condition == "finecon" :
            ...
            for ep in range(n_epochs) :
                ...
                for batch in data_gen :
                    ...
                    gradients = tape.gradient(total_loss, backbone.trainable_variables+classifier.trainable_variables+head.trainable_variables)
                    optim.apply_gradients(zip(gradients, backbone.trainable_variables+classifier.trainable_variables+head.trainable_variables))
