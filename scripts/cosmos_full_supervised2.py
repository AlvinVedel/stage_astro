...
from deep_models import basic_backbone, treyer_backbone, astro_head, astro_model, AstroModel, adv_network, AstroFinetune
...
for base in base_names :
    ...
    model = AstroFinetune(basic_backbone(full_bn=False, all_bn=False), astro_head(1024, 400), train_back=True)
    model(np.random.random((32, 64, 64, 6)))
    gen = SupervisedGenerator(base_path+base+".npz", batch_size=32, adversarial=False, adversarial_dir=adv_path, adv_extensions=["_D.npz"], nbins=400, apply_log=True)
