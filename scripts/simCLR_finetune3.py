...
save_w_path = "model_save/checkpoints_simCLR_finetune2/simCLR_finetune_UD_D_log"
plots_path = "plots/simCLR/simCLR_finetune/"

for base in ["base1", "base2", "base3"] :

    data_gen = SupervisedGenerator("/lustre/fswork/projects/rech/dnz/ull82ct/astro/data/finetune/"+base+".npz", batch_size=32, nbins=400, apply_log=True)
    ...
    model1.fit(data_gen, epochs=5)   #### ON PRE ENTRAINE LA TETE
    ...
    print("TRAIN ENDED FOR ", base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_gradclip3.weights.h5")
    """
    plt.plot(np.arange(1, n_epochs+1), history.history["reg_loss"])
    plt.xlabel("epochs")
    plt.ylabel("loss (mae)")
