...
for i, finetune_base in enumerate(["b1_1", "b2_1", "b3_1"]) :   #### POUR CHAQUE CONDITION D ENTRAINEMENT ON VA AVOIR UN SUBPLOT
    ...
    for j, inf_base in enumerate(base_liste):
        ...
        simbases = ["ViT"] #    , "UD800_classif","UD_D800_classif"]
        iter=0
        ...
        for k in range((len(simbases)*2)-1) :
            if k == 0 :
                ...
                model_name = "../model_save/checkpoints_supervised/vit_backbone_supervised_v2_"+finetune_base+".weights.h5"
                tag_name = "supervised_vit"
                treyer = True
                model = ViT_model()
