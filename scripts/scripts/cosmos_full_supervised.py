...
def ViT_model() :
    back = ViT_backbone(patch_size=4, gp='average')
    print("model creaating")
    test = back(np.random.random((32, 64, 64, 5)))
    print("test sucessful, value propaged")
    print(test[0])
...
for base in base_names :
    if base == "b1_1" :
        val_gen = DataGen(base_path+base_names[1]+"_v2.npz", batch_size=32)
    else :
        val_gen = DataGen(base_path+base_names[0]+"_v2.npz", batch_size=32)
    #model = backbone()
    model = ViT_model()
    ...
    history = model.fit(gen, epochs=n_epochs, callbacks=[LearningRateScheduler()], validation_data=(val_gen.images[:, :, :, :5], [val_gen.z_bins, val_gen.z_values]))
