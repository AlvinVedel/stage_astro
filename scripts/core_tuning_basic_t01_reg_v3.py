...
from contrastiv_model import simCLR, simCLRcolor1, simCLRcolor1_adversarial, simCLR1, NTXent, CoreTuning, CoreTuningv2
from deep_models import basic_backbone, projection_mlp, color_mlp, segmentor, deconvolutor, astro_head, astro_model, classif_mlp, AstroFinetune, noregu_projection_mlp, classif_astro_head
import os 
from vit_layers import ViT_backbone
from generator import SupervisedGenerator, COINGenerator, MultiGen, CoreGenerator
...
save_w_path = "model_save/simCLR_finetune_comparaison/basic_color_t01_reg_fullClassif_coreV2"
...
for base in ["base1", "base2", "base3"] :
    ...
    data_gen = CoreGenerator("/lustre/fswork/projects/rech/dnz/ull82ct/astro/data/finetune/"+base+".npz", batch_size=256, nbins=400, apply_log=True)
    n_epochs = 100

    for condition in ["coretuning", "sup"] :
        ...
        if condition == "sup" :
            ...
            classifier = classif_astro_head(in_dim, 400)
            ...
            for ep in range(n_epochs) :
                ...
                for i, batch in enumerate(data_gen) :
                    ...
                    with tf.GradientTape() as tape :
                        ...
                        pdf_loss = tf.reduce_mean(tf.keras.losses.sparse_categorical_crossentropy(labels_dict["pdf"], pdf))
                        #reg_loss = tf.reduce_mean(tf.math.sqrt(tf.reduce_sum( (labels_dict["reg"] - reg)**2, axis=1) ), axis=0)
                        core_loss = coretuning_loss.call(z, labels_dict["core"])


                        total_loss = pdf_loss+core_loss
                    gradients = tape.gradient(total_loss, back.trainable_variables+classifier.trainable_variables+proj.trainable_variables)
                    optim.apply_gradients(zip(gradients, back.trainable_variables+classifier.trainable_variables+proj.trainable_variables))
