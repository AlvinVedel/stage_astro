...
from contrastiv_model import simCLR, simCLRcolor1, simCLRcolor1_adversarial, simCLR1
...
model_name = "norm300_ColorHead_Regularized"
...
for base in ["base1", "base2", "base3"] :
    ...
    #model1 = AstroFinetune(extracteur, astro_head(1024, 400), train_back=False)
    ...
    model1 = AstroFinetune(extracteur, astro_head(1024, 400), train_back=False)
    model1.compile(optimizer=keras.optimizers.Adam(1e-4))
    model1.fit(data_gen, epochs=15)   #### ON PRE ENTRAINE LA TETE
    ## RE ENTRAINEEMNT DU MODEL EN ENTRAINANT LE BACK CETTE FOIS
    model1.train_back=True
    model1.compile(optimizer=keras.optimizers.Adam(learning_rate=5e-7, clipnorm=1e-3))

    #model1.compile(optimizer=keras.optimizers.Adam(1e-4), loss={"pdf" : tf.keras.losses.SparseCategoricalCrossentropy(), "reg":tf.keras.losses.MeanAbsoluteError()}, metrics= {"pdf":["accuracy"], "reg" :[Bias(name='global_bias'), SigmaMAD(name='global_smad'), OutlierFraction(name='global_outl'),Bias(inf=0, sup=0.4, name='bias1'), Bias(inf=0.4, sup=2, name='bias2'), Bias(inf=2, sup=4, name='bias3'), Bias(inf=4, sup=6, name='bias4'), 
    #              SigmaMAD(inf=0, sup=0.4, name='smad1'), SigmaMAD(inf=0.4, sup=2, name='smad2'), SigmaMAD(inf=2, sup=4, name='smad3'), SigmaMAD(inf=4, sup=6, name='smad4'), OutlierFraction(inf=0, sup=0.4, name='outl1'), OutlierFraction(inf=0.4, sup=2, name='outl2'), OutlierFraction(inf=2, sup=4, name='outl3'), OutlierFraction(inf=4, sup=6, name='outl4')]})
    
    history = model1.fit(data_gen, epochs=n_epochs, callbacks=[AlternateTreyerScheduler(eps=[35, 45])])
    model1.save_weights(base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr57_clip3_ep50.weights.h5")

    print("TRAIN ENDED FOR ", base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr6_clip3.weights.h5")
    ...
    ## RE ENTRAINEEMNT DU MODEL EN ENTRAINANT LE BACK CETTE FOIS
    model1.train_back=True
    model1.compile(optimizer=keras.optimizers.Adam(5e-7, clipnorm=1e-3))

    #model1.compile(optimizer=keras.optimizers.Adam(1e-4), loss={"pdf" : tf.keras.losses.SparseCategoricalCrossentropy(), "reg":tf.keras.losses.MeanAbsoluteError()}, metrics= {"pdf":["accuracy"], "reg" :[Bias>    #              SigmaMAD(inf=0, sup=0.4, name='smad1'), SigmaMAD(inf=0.4, sup=2, name='smad2'), SigmaMAD(inf=2, sup=4, name='smad3'), SigmaMAD(inf=4, sup=6, name='smad4'), OutlierFraction(inf=0, sup=0.4, n>
    history = model1.fit(data_gen, epochs=n_epochs*2, callbacks=[AlternateTreyerScheduler(eps=[70, 90])])
    model1.save_weights(base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr57_clip3_ep100.weights.h5")

    print("TRAIN ENDED FOR ", base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr7_clip3.weights.h5")
    ...
    #model1.compile(optimizer=keras.optimizers.Adam(1e-4), loss={"pdf" : tf.keras.losses.SparseCategoricalCrossentropy(), "reg":tf.keras.losses.MeanAbsoluteError()}, metrics= {"pdf":["accuracy"], "reg" :[Bias>    #              SigmaMAD(inf=0, sup=0.4, name='smad1'), SigmaMAD(inf=0.4, sup=2, name='smad2'), SigmaMAD(inf=2, sup=4, name='smad3'), SigmaMAD(inf=4, sup=6, name='smad4'), OutlierFraction(inf=0, sup=0.4, n>
    history = model1.fit(data_gen, epochs=n_epochs*3, callbacks=[AlternateTreyerScheduler(eps=[105, 135])])
    model1.save_weights(base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr57_clip3_ep150.weights.h5")

    print("TRAIN ENDED FOR ", base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr6_clip4.weights.h5")
    ...
    model1 = AstroFinetune(extracteur, astro_head(1024, 400), train_back=False)
    model1.compile(optimizer=keras.optimizers.Adam(1e-4))
    model1.fit(data_gen, epochs=10)   #### ON PRE ENTRAINE LA TETE
    ## RE ENTRAINEEMNT DU MODEL EN ENTRAINANT LE BACK CETTE FOIS
    model1.train_back=True
    model1.compile(optimizer=keras.optimizers.Adam(5e-7, clipnorm=1e-4))

    #model1.compile(optimizer=keras.optimizers.Adam(1e-4), loss={"pdf" : tf.keras.losses.SparseCategoricalCrossentropy(), "reg":tf.keras.losses.MeanAbsoluteError()}, metrics= {"pdf":["accuracy"], "reg" :[Bias>    history = model1.fit(data_gen, epochs=n_epochs, callbacks=[TreyerScheduler()])
    model1.save_weights(base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr7_clip4.weights.h5")

    print("TRAIN ENDED FOR ", base_path+save_w_path+"_ALL_base="+base+"_model="+model_name+"_HYP=lr7_clip4.weights.h5")
    """
